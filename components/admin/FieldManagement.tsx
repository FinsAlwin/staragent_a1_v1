...
const FieldManagement: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEditing && currentField.id) {
      dispatch({ type: 'UPDATE_EXTRACTION_FIELD', payload: currentField as ExtractionField });
    } else {
      dispatch({ type: 'ADD_EXTRACTION_FIELD', payload: { ...currentField, id: Date.now().toString() } as ExtractionField });
    }
    closeModal();
  };
  ...
  return (
    <div className="p-6 bg-white shadow-lg rounded-lg">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-2xl font-semibold text-gray-700">Manage Extraction Fields</h3>
        <button
      ...
      {extractionFields.length === 0 ? (
        ...
        <ul className="space-y-3">
          {extractionFields.map((field: ExtractionField) => (
            <li key={field.id} className="p-4 border border-gray-200 rounded-md flex flex-col sm:flex-row justify-between sm:items-center hover:bg-gray-50 transition-colors">
              <div className="mb-2 sm:mb-0">
                <p className="font-semibold text-gray-800">{field.label} <span className="text-sm text-gray-500">({field.key})</span></p>
                {field.description && <p className="text-sm text-gray-600">{field.description}</p>}
              </div>
              <div className="space-x-2 flex-shrink-0">
                <button onClick={() => openModalForEdit(field)} className="text-blue-500 hover:text-blue-700 font-medium px-2 py-1">Edit</button>
                <button onClick={() => handleDelete(field.id)} className="text-red-500 hover:text-red-700 font-medium px-2 py-1">Delete</button>
              </div>
            </li>
          ))}
        </ul>
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title={isEditing ? "Edit Extraction Field" : "Add Extraction Field"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="label" className="block text-sm font-medium text-gray-700">Field Label (User-facing)</label>
            <input
              type="text"
              name="label"
              id="label"
          ...
          <div>
            <label htmlFor="key" className="block text-sm font-medium text-gray-700">Field Key (for JSON, auto-generated)</label>
            <input
              type="text"
              name="key"
              id="key"
            ...
            />
             <p className="text-xs text-gray-500 mt-1">This key is used in the JSON output. It's generated from the label.</p>
          </div>
          <div>
            <label htmlFor="description" className="block text-sm font-medium text-gray-700">Description (Optional, guidance for AI)</label>
            <textarea
              name="description"
              id="description"
